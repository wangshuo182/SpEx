...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for iter, (mixture, oracle_s, oracle_ids, _) in enumerate(tqdm(self.train_dataloader, desc="Training")):
            ...
            self.writer.add_scalars("train/Loss", {
                ...
                'loss': loss.item()
            }, iter + (epoch-1)*len(self.train_dataloader))
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (mixture, oracle_s, oracle_ids, filename) in tqdm(enumerate(self.validation_dataloader)):
            ...
            samples['sdr_improve'].append(c_e - c_m)
            samples['name'].append(filename)

            # visualize spec / audio
            self._visuliza_spec_audio(epoch, i, mixture, s1, s2, s1_clean, s2_clean, filename, c_e, prefix='val_')
        
        # visualize spk_id_distribution
        ...
        self.writer.add_scalar("Metrics/val_improved_SDR", get_metrics_ave(samples['sdr_improve']), epoch)
        return get_metrics_ave(samples['sdr_improve'])
    ...
    def _inference(self, epoch=0):
        ...
        for i, (mixture, oracle_s, oracle_ids, filename) in tqdm(enumerate(self.test_dataloader)):
            ...
            self._visuliza_spec_audio(epoch, i, mixture, s1, s2, s1_clean, s2_clean, filename, c_e, prefix='test_')
        self.writer.add_scalar("Metrics/test_improved_SDR", get_metrics_ave(samples['sdr_improve']), epoch)
        return get_metrics_ave(samples['sdr_improve'])
    ...
    def _save_checkpoint(self, epoch, is_best=False):
        ...
        state_dict["model"]["sep"] = net.sep_stack.state_dict()
