...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for iter, (mixture, oracle_s, oracle_ids, _) in enumerate(tqdm(self.train_dataloader, desc="Training")):
            ...
            spk_vectors = self.model.get_speaker_vectors(mixture)
            ...
            loss_step_sum += loss.item()

            if iter % 10 == 0 and iter > 0 :
                self.writer.add_scalars(f"Train/Loss", {'epoch_{}'.format(epoch): loss_step_sum / 10}, iter)
                loss_step_sum = 0.0
