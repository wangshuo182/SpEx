...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Random start
        if self.like_test:
            rand_start, stop = 0, None
        else:
            if sample_len <= self.seg_len:
                rand_start = 0
            else:
        ...
        return torch.from_numpy(x), sources, torch.Tensor(spkid).long(), filename

    def get_spk2indx(self):
        return self.spk2indx
