...
class Conv1DBlock(nn.Module):
    def __init__(self, in_chan, hid_chan, kernel_size, padding, dilation, norm_type="gLN"):
        ...
        # layer_norm = nn.LayerNorm(hid_chan)
        ...
        # self.out = nn.Sequential(depth_conv1d, nn.PReLU(), layer_norm)
...
class SepConv1DBlock(nn.Module):
    def __init__(
        ...
        # self.out = nn.Sequential(nn.PReLU(), layer_norm)
        # FiLM conditioning
        if self.use_FiLM:
